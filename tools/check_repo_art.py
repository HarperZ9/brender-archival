...
of a spec that IS diffable, so this re-renders it and compares bytes. Bytes, not
decoded text: a checkout that rewrote the line endings reads back identical
through Python's universal newlines and would pass a text comparison, so
docs/art is pinned to LF in .gitattributes and the comparison reads the file as
it sits on disk.
...
ART = ROOT / "docs" / "art"
SCHEMA = "brender-archival.repo-art/v1"
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "brender-archival", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('brender-archival') % 100000:05d}"
